...
class GaussianKernel(Kernel):
    ...
    def kernel(self, X: ndarray, Y: ndarray | None = None):
        ...
        if flag == 0:
            ...
            X_features = rbf_feature.fit_transform(
                X
            )  # print(X_features.shape, X_features.T.shape)
            ...
            feature_map_nystroem = Nystroem(
                gamma=0.2, n_components=100, random_state=1
            )  # gamma=0.2
            X_features = feature_map_nystroem.fit_transform(
                X
            )  # print(X_features.shape, X_features.T.shape)
            K = X_features @ X_features.T  # print(K.shape)
            # -----------------------------------------------------------------------------------
