...
def pdag2dag(G: GeneralGraph) -> GeneralGraph:
    ...
    for edge in edges:
        if not (
            (edge.endpoint1 == Endpoint.ARROW and edge.endpoint2 == Endpoint.TAIL)
            or (edge.endpoint1 == Endpoint.TAIL and edge.endpoint2 == Endpoint.ARROW)
        ):
            Gd.remove_edge(edge)

    Gp = deepcopy(G)
    inde = np.zeros(
        Gp.num_vars, dtype=np.dtype(int)
    )  # index whether the ith node has been removed. 1:removed; 0: not
    while 0 in inde:
        for i in range(Gp.num_vars):
            if inde[i] == 0:
                ...
                if (
                    len(
                        np.intersect1d(
                            np.where(Gp.graph[:, i] == 1)[0], np.where(inde == 0)[0]
                        )
                    )
                    == 0
                ):  # Xi has no out-going edges
                    sign = sign + 1
                    Nx = np.intersect1d(
                        np.intersect1d(
                ...
                if sign == 2:
                    ...
                    for index in np.intersect1d(
                        np.where(Gp.graph[:, i] == -1)[0],
                        np.where(Gp.graph[i, :] == -1)[0],
                    ):
                        Gd.add_edge(
                            Edge(nodes[index], nodes[i], Endpoint.TAIL, Endpoint.ARROW)
                        )
                    inde[i] = 1
