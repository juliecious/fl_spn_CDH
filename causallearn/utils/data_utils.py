...
def my_simulate_general_hetero(W, K, n, sem_type):
    ...
    # choice = [1]
    print(
        f"In general cases. The changing variable is: {choice}. The order is: {ordered_vertices}."
    )
    ...
    for j in ordered_vertices:
        ...
        if j in choice:  # changes
            ...
            for k in range(K):
                ...
                b_ = b_ * W
                X[k * sample : (k + 1) * sample, j] = (
                    f(X[k * sample : (k + 1) * sample, parents]) @ b_[parents, j]
                )
            X[:, j] = X[:, j] + sigma * eps
...
def my_simulate_linear_gaussian(W, K, n, sem_type):
    ...
    # choice = [1]
    print(
        f"This is Linear Gaussian Model. The changing variable is: {choice}. The order is: {ordered_vertices}."
    )
    ...
    for j in ordered_vertices:
        ...
        if j in choice:  # changes
            ...
            for k in range(K):
                ...
                b_ = b_ * W
                X[k * sample : (k + 1) * sample, j] = (
                    X[k * sample : (k + 1) * sample, parents] @ b_[parents, j]
                )
            X[:, j] = X[:, j] + sigma * eps
...
def my_count_accuracy(graph, B_true):
    ...
    extra_lower = np.setdiff1d(
        pred_lower, cond_lower, assume_unique=True
    )  # value in arr1, not in arr2.
    missing_lower = np.setdiff1d(cond_lower, pred_lower, assume_unique=True)
    ...
    false_pos = np.setdiff1d(
        pred, true, assume_unique=True
    )  # false positve includes no directions, wrong directions or reverse directions.
    ...
    return {"shd": shd, "tpr": tpr, "fdr": fdr, "fpr": fpr, "fpr2": fpr_2}, [
        shd,
        tpr,
        fdr,
        fpr,
...
def count_skeleton_accuracy(B_bin_true, B_bin_est):
    skeleton_true = get_skeleton(
        B_bin_true
    )  # b_bin_true[i,j]=1  <==> skeleton[i,j]=skeleton[j,i]=1
    skeleton_est = get_skeleton(
        B_bin_est
    )  # b_bin_est[i,j]=-1 & b_bin_est[j,i]=1  <==>  skeleton[i,j]=skeleton[j,i]=1
    ...
    false_neg = np.setdiff1d(
        cond, pred, assume_unique=True
    )  # This is also OK: np.setdiff1d(cond, true_pos, assume_unique=True)
    ...
    fdr = float(len(false_pos)) / max(
        nnz, 1
    )  # fdr = (FP) / (TP + FP) = FP / |pred_graph|
    tpr = float(len(true_pos)) / max(
        len(cond), 1
    )  # tpr: TP / (TP + FN) = TP / |true_graph|
    fpr = float(len(false_pos)) / max(
        cond_neg_size, 1
    )  # fpr: (FP) / (TN + FP) = FP / ||
    try:
...
def get_dag_from_pdag(B_bin_pdag):
    ...
    if np.all(B_bin_pdag == 0):
        ...
        (
            ...
            G.to_dag().to_amat()
        )  # The key is: to_dag() - converting a PDAG to a DAG using some rules.
