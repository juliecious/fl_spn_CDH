...
class FedPC(CIT_Base):
    """
    Oracle Independence Test using a pre-trained Network-Aligned Federated Probabilistic Circuit (FedPC).
    Replaces standard statistical tests (KCI/FisherZ) with model-based CMI queries.
    """

    def __init__(self, fed_pc_model, data, threshold=0.05, **kwargs):
        """
        Args:
            fed_pc_model (FedPC): The trained Federated Probabilistic Circuit.
            threshold (float): CMI threshold. If CMI > threshold, we reject independence (p=0).
        """
        super().__init__(data, **kwargs)
        self.fed_pc = fed_pc_model
        self.threshold = threshold
        # The 'method' attribute is required by CDNOD/PC to identify the test type
        self.method = "fedpc_oracle"

    def __call__(self, X, Y, Z, data_matrix=None):
        """
        Perform Conditional Independence Test: X _|_ Y | Z

        Args:
            X (int): Index of variable X.
            Y (int): Index of variable Y.
            Z (list): List of indices for conditioning set Z.
            data_matrix (np.ndarray, optional): The data matrix.
                                                Used if FedPC calculates empirical CMI (averaging over data).

        Returns:
            p_value (float):
                0.0 if Dependent (CMI > Threshold) -> Reject Null Hypothesis
                1.0 if Independent (CMI <= Threshold) -> Accept Null Hypothesis
        """
        # Type safety for indices
        if isinstance(Z, (int, float, np.integer)):
            Z = [int(Z)]
        Z = list(Z)

        # 1. Delegate CMI calculation to the FedPC model
        # The model uses its learned density P(V) to estimate I(X;Y|Z)
        cmi_val = self.fed_pc.calculate_cmi(X, Y, Z, data_matrix)

        # 2. Oracle Thresholding
        # In a parametric oracle setting, we don't return a smooth p-value.
        # We make a binary decision based on the information flow magnitude.
        if cmi_val > self.threshold:
            return 0.0  # Dependent (Reject H0)
        else:
            return 1.0  # Independent (Accept H0)
