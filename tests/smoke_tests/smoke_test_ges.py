#!/usr/bin/env python
"""
Quick Smoke Test: GES (Greedy Equivalence Search)

Tests the GES algorithm (centralized baseline) on a tiny synthetic dataset.

Configuration:
- Dataset: Synthetic ER graph (5 nodes, 200 samples)
- Method: GES (score-based)
- Device: CPU only
...
def plot_graph(adj_matrix, title, output_path, node_labels=None):
    ...
    nx.draw_networkx_nodes(G, pos, node_color="lightgreen", node_size=1000, alpha=0.9)
...
def main():
    logging.info("=" * 80)
    logging.info("GES SMOKE TEST - Greedy Equivalence Search (Centralized)")
    logging.info("=" * 80)

    d, n, seed = 5, 200, 42
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = Path(__file__).parent / f"{timestamp}_ges_{d}vars_{n}samples"
    output_dir.mkdir(exist_ok=True)

    logging.info(f"Configuration: d={d}, n={n}, seed={seed}")
    ...
    G_pred = record["G"].graph

    plot_graph(G_pred, "Discovered DAG (GES)", output_dir / "discovered_graph.png")
    ...
    results = {
        "method": "GES",
        "timestamp": timestamp,
        "config": {"d": d, "n": n, "seed": seed, "device": "cpu"},
        "metrics": {
    ...
    with open(output_dir / "summary.txt", "w") as f:
        f.write("GES SMOKE TEST SUMMARY\n")
        f.write("=" * 80 + "\n\n")
